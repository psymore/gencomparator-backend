/*
  Warnings:

  - Made the column `userId` on table `api_key` required. This step will fail if there are existing NULL values in that column.

*/
-- DropForeignKey
ALTER TABLE "api_key" DROP CONSTRAINT "api_key_userId_fkey";

-- AlterTable
ALTER TABLE "api_key" ALTER COLUMN "userId" SET NOT NULL;

-- AddForeignKey
ALTER TABLE "api_key" ADD CONSTRAINT "api_key_userId_fkey" FOREIGN KEY ("userId") REFERENCES "User"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
